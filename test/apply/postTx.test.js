...
test('handleApplyTxOperation (apply) - negative', t => {
    ...
    t.test('handleApplyTxOperation (apply) - invalid txo sub-values', async t => {
        ...
        for (let i = 0; i < testCases.length; i++) {
            ...
            const maliciousWriter = await setupMsbWriter(admin, `writer${i}`, testCase.writer, tmpDirectory, admin.options);
            ...
            await tryToSyncWriters(maliciousWriter, admin);
            // should be penalized
            const node = await admin.msb.state.getNodeEntry(maliciousWriter.wallet.address)
