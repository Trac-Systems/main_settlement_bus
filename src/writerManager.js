...
import {MAX_PUBKEYS_LENGTH, WHITELIST_FILEPATH, OperationType} from './constants.js';
...
export class MsbManager extends ReadyResource {
    ...
    static assembleAdminMessage(adminEntry, writingKey, wallet, bootstrap) {
        ...
        if (!adminEntry && wallet && writingKey && writingKey === bootstrap) {
            ...
            const msg = this.createMessage(wallet.publicKey, writingKey ,nonce, OperationType.ADD_ADMIN);
    ...
    static assembleAddWriterMessage(wallet, writingKey) {
        const nonce = this.#generateNonce();
        const msg = this.createMessage(wallet.publicKey, writingKey, nonce, OperationType.ADD_WRITER);
        const hash = createHash('sha256').update(msg).digest('hex');
    ...
    static assembleRemoveWriterMessage(wallet, writingKey) {
        const nonce = this.#generateNonce();
        const msg = this.createMessage(wallet.publicKey, writingKey, nonce, OperationType.REMOVE_WRITER);
        const hash = createHash('sha256').update(msg).digest('hex');
    ...
    static verifyEventMessage(parsedRequest, wallet) {
        let msg = null;
        if (parsedRequest.type === OperationType.ADD_ADMIN || parsedRequest.type === OperationType.REMOVE_WRITER) {
