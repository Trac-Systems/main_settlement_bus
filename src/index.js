...
export class MainSettlementBus extends ReadyResource {
    ...
    #initInternalAttributes(options) {
        ...
        this.#KEY_PAIR_PATH = `${this.#STORES_DIRECTORY}${options.store_name
            }/db/keypair.json`;
    ...
    async _open() {
        ...
        if (this.#isAdmin(adminEntry)) {
            ...
            !this.#isAdmin(adminEntry) &&
            this.#state.isWritable() &&
            !this.#state.isIndexer()
        ) {
    ...
    async #handleIncomingEvent(bufferedRequest) {
        try {
            ...
            if (decodedRequest.type) {
                if (
                    decodedRequest.type === OperationType.ADD_WRITER ||
                    decodedRequest.type === OperationType.REMOVE_WRITER
                ) {
