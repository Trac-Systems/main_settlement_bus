...
class V1BroadcastTransactionResponse extends V1BaseOperation {
    ...
    async assertProofPayloadMatchesRequestPayload(proofResult, pendingRequestServiceEntry) {
        ...
        const stippedValidatorPayload = stripValidatorMetadata(stateTxDecodedFromResponse);

        if (!isDeepEqualApplyPayload(stateTxDecodedFromRequest, stippedValidatorPayload)) {
