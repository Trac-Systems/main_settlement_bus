...
class TransferHandler extends BaseHandler {
    ...
    async #transfer(senderAddressString, recipientAddressString, validatorAddressString, validatorEntryBuffer, transferAmountBuffer, feeAmountBuffer, isSelfTransfer, isRecipientValidator, batch, node) {
        if (!senderAddressString ||
            ...
            this.logger.error(OperationType.TRANSFER, "Invalid transfer incoming data.", node.from.key)
            return null;
        }
        ...
        if (transferAmount === null || feeAmount === null) {
            this.logger.error(OperationType.TRANSFER, "Invalid fee/transfer amount.", node.from.key)
            return null;
        }
        ...
        if (totalDeductedAmount === null) {
            this.logger.error(OperationType.TRANSFER, "Invalid total deducted amount.", node.from.key)
            return null;
        }

        const senderEntryBuffer = await this.#repo.getEntry(senderAddressString, batch);
        if (senderEntryBuffer === null) {
            this.logger.error(OperationType.TRANSFER, "Invalid sender node entry buffer.", node.from.key)
            return null;
        }

        const senderEntry = nodeEntryUtils.decode(senderEntryBuffer);
        if (senderEntry === null) {
            this.logger.error(OperationType.TRANSFER, "Invalid sender node entry.", node.from.key)
            return null;
        }

        const senderBalance = toBalance(senderEntry.balance);
        if (senderBalance === null) {
            this.logger.error(OperationType.TRANSFER, "Invalid sender balance.", node.from.key)
            return null;
        }

        if (!senderBalance.greaterThanOrEquals(totalDeductedAmount)) {
            this.logger.error(OperationType.TRANSFER, "Insufficient sender balance.", node.from.key)
            return Status.IGNORE;
        }

        const newSenderBalance = senderBalance.sub(totalDeductedAmount);
        if (newSenderBalance === null) {
            this.logger.error(OperationType.TRANSFER, "Failed to apply fee to sender node balance.", node.from.key)
            return null;
        }

        const updatedSenderEntry = newSenderBalance.update(senderEntryBuffer);
        if (updatedSenderEntry === null) {
            this.logger.error(OperationType.TRANSFER, "Failed to update sender node balance.", node.from.key)
            return null;
        }
        ...
        if (!isSelfTransfer && !isRecipientValidator) {
            ...
            if (recipientEntryBuffer === null) {
                if (transferAmount.value === null) {
                    this.logger.error(OperationType.TRANSFER, "Invalid transfer amount.", node.from.key)
                    return null;
                };
                ...
                if (newRecipientEntry.length === 0) {
                    this.logger.error(OperationType.TRANSFER, "Invalid recipient entry.", node.from.key)
                    return null;
                };
                ...
                if (recipientBalance === null) {
                    this.logger.error(OperationType.TRANSFER, "Invalid recipient balance.", node.from.key)
                    return null;
                };

                const newRecipientBalance = recipientBalance.add(transferAmount);
                if (newRecipientBalance === null) {
                    this.logger.error(OperationType.TRANSFER, "Failed to transfer amount to recipient balance.", node.from.key)
                    return null;
                };

                const updatedRecipientEntry = newRecipientBalance.update(recipientEntryBuffer);
                if (updatedRecipientEntry === null) {
                    this.logger.error(OperationType.TRANSFER, "Failed to update recipient node balance.", node.from.key)
                    return null;
                };
        ...
        if (validatorEntry === null) {
            this.logger.error(OperationType.TRANSFER, "Invalid validator entry.", node.from.key)
            return null;
        }
        ...
        if (newValidatorBalance === null) {
            this.logger.error(OperationType.TRANSFER, "Failed to transfer fee to validator balance.", node.from.key)
            return null;
        }

        const updatedValidatorEntry = newValidatorBalance.update(validatorEntryBuffer);
        if (updatedValidatorEntry === null) {
            this.logger.error(OperationType.TRANSFER, "Failed to update validator node balance.", node.from.key)
            return null;
        }
    ...
    async performOperation(op, view, base, node, batch) {
        if (!this.#stateValidationSchema.validateTransferOperation(op)) {
            this.logger.error(OperationType.TRANSFER, "Contract schema validation failed.", node.from.key)
        ...
        if (!Object.hasOwn(op.tro, "vs") || !Object.hasOwn(op.tro, "va") || !Object.hasOwn(op.tro, "vn")) {
            this.logger.error(OperationType.TRANSFER, "Operation is not complete.", node.from.key)
            return Status.FAILURE;
        };
        // for additional security, nonces should be different.
        if (b4a.equals(op.tro.in, op.tro.vn)) {
            this.logger.error(OperationType.TRANSFER, "Nonces should not be the same.", node.from.key)
            return Status.FAILURE;
        };
        // addresses should be different.
        if (b4a.equals(op.address, op.tro.va)) {
            this.logger.error(OperationType.TRANSFER, "Addresses should not be the same.", node.from.key)
            return Status.FAILURE;
        };
        // signatures should be different.
        if (b4a.equals(op.tro.is, op.tro.vs)) {
            this.logger.error(OperationType.TRANSFER, "Signatures should not be the same.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (requesterAddressString === null) {
            this.logger.error(OperationType.TRANSFER, "Requester address is invalid.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(requesterPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.TRANSFER, "Error while decoding requester public key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (requesterMessage.length === 0) {
            this.logger.error(OperationType.TRANSFER, "Invalid requester message.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!b4a.equals(regeneratedTxHash, op.tro.tx)) {
            this.logger.error(OperationType.TRANSFER, "Message hash does not match the tx_hash.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!isRequesterSignatureValid) {
            this.logger.error(OperationType.TRANSFER, "Failed to verify message signature.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (validatorAddressString === null) {
            this.logger.error(OperationType.TRANSFER, "Validator address is invalid.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(validatorPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.TRANSFER, "Failed to decode validator public key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (validatorMessage.length === 0) {
            this.logger.error(OperationType.TRANSFER, "Invalid validator message.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (indexersSequenceState === null) {
            this.logger.error(OperationType.TRANSFER, "Indexer sequence state is invalid.", node.from.key)
            return Status.FAILURE;
        };

        if (!b4a.equals(op.tro.txv, indexersSequenceState)) {
            this.logger.error(OperationType.TRANSFER, "Transaction was not executed.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!isValidatorValid) {
            this.logger.error(OperationType.TRANSFER, "Validator consistency check failed.", node.from.key)
            return Status.FAILURE;
        }
        ...
        if (opEntry !== null) {
            this.logger.error(OperationType.TRANSFER, "Operation has already been applied.", node.from.key)
            return Status.IGNORE;
        };
        ...
        if (recipientAddressString === null) {
            this.logger.error(OperationType.TRANSFER, "Invalid recipient address.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(recipientPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.TRANSFER, "Failed to decode recipient public key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (transferResult === null) {
            this.logger.error(OperationType.TRANSFER, "Invalid transfer result.", node.from.key);
            return Status.FAILURE;
        }

        if (transferResult === Status.IGNORE) {
            this.logger.error(OperationType.TRANSFER, "Transfer operation skipped.", node.from.key);
            return Status.IGNORE;
        };

        if (transferResult.senderEntry === null) {
            this.logger.error(OperationType.TRANSFER, "Invalid sender entry.", node.from.key)
            return Status.FAILURE;
        };
