...
class BalanceInitializationHandler extends BaseHandler {
    #repo;
    #config;
    ...
    async performOperation(op, view, base, node, batch) {
        if (!this.#stateValidationSchema.validateBalanceInitialization(op)) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Contract schema validation failed.", node.from.key)
        ...
        if (adminAddressString === null) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Requester address is invalid.", node.from.key)
            return Status.FAILURE;
        }
        ...
        if (b4a.equals(requesterAdminPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Error while decoding requester public key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (recipientAddressString === null) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Recipient address is invalid.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(recipientPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Failed to decode recipient public key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (amount === null) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Invalid balance.", node.from.key)
            return Status.FAILURE;
        };

        // Entry has been disabled so there is nothing to do
        if (await this.#repo.isInitalizationDisabled(batch)) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Balance initialization is disabled.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (decodedAdminEntry === null) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Failed to decode admin entry.", node.from.key)
            return Status.FAILURE;
        }

        if (!this.#repo.isAdmin(decodedAdminEntry, node)) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Node is not allowed to perform this operation. (ADMIN ONLY)", node.from.key)
            return Status.FAILURE;
        };

        const adminPublicKey = tracCryptoApi.address.decodeSafe(decodedAdminEntry.address);
        if (b4a.equals(adminPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Failed to decode admin public key.", node.from.key)
            return Status.FAILURE;
        };

        // Admin consistency check
        if (!b4a.equals(adminPublicKey, requesterAdminPublicKey)) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "System admin and node public keys do not match.", node.from.key)
            return Status.FAILURE;
        }
        ...
        if (message.length === 0) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Invalid requester message.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!b4a.equals(hash, op.bio.tx)) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Message hash does not match the tx_hash.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!isMessageVerified) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Failed to verify message signature.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (indexersSequenceState === null) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Indexer sequence state is invalid.", node.from.key)
            return Status.FAILURE;
        };

        if (!b4a.equals(op.bio.txv, indexersSequenceState)) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Transaction was not executed.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (opEntry !== null) {
            this.logger.error(OperationType.BALANCE_INITIALIZATION, "Operation has already been applied.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (incomingAddressNodeEntryBuffer === null) {
            ...
            if (nodeEntry.length === 0) {
                this.logger.error(OperationType.BALANCE_INITIALIZATION, "Failed to initialize node entry.", node.from.key)
                return Status.FAILURE;
            }
            ...
            if (nodeEntry === null) {
                this.logger.error(OperationType.BALANCE_INITIALIZATION, "Failed to set node entry balance.", node.from.key)
                return Status.FAILURE;
            }
        };
