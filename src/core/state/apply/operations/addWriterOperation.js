...
class AddWriterHandler {
    ...
    async #addWriter(op, base, node, batch, txHashHexString, requesterAddressString, requesterAddressBuffer, validatorAddressString, validatorEntryBuffer) {
        ...
        if (requesterNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to verify requester node address.", node.from.key)
            return null;
        };

        const decodedRequesterNodeEntry = nodeEntryUtils.decode(requesterNodeEntry)
        if (decodedRequesterNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to decode node entry.", node.from.key)
            return null;
        };
        ...
        if (writerKeyHasBeenRegistered !== null) {
            ...
            if (!isCurrentWk || !isOwner) {
                this.#repo.safeLog(OperationType.ADD_WRITER, "Invalid writer key: either not owned by requester or different from assigned key.", node.from.key)
                return null;
            }
        }
        ...
        if (isIndexer || isWriter || !isWhitelisted) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Node must be whitelisted, and cannot be a writer or an indexer.", node.from.key)
            return null;
        };
        ...
        if (requesterBalance === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Invalid requester balance.", node.from.key)
            return null;
        };

        if (!requesterBalance.greaterThanOrEquals(BALANCE_FEE)) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Insufficient requester balance.", node.from.key)
            return Status.IGNORE;
        };

        const updatedBalance = requesterBalance.sub(BALANCE_FEE) // Remove the fee
        if (updatedBalance === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to apply fee to requester balance.", node.from.key)
            return null;
        };
        ...
        if (updatedRoleRequesterNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to update node entry with a writer role.", node.from.key)
            return null;
        };
        ...
        if (chargedFeeRequesterNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to update node balance.", node.from.key)
            return null;
        };
        ...
        if (decodedValidatorEntry === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to decode validator entry.", node.from.key)
            return null;
        };

        const validatorBalance = toBalance(decodedValidatorEntry.balance)
        if (validatorBalance === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Invalid validator balance.", node.from.key)
            return null;
        };

        const updatedValidatorBalance = validatorBalance.add(BALANCE_FEE.percentage(PERCENT_75))
        if (updatedValidatorBalance === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to transfer fee to validator.", node.from.key)
            return null;
        };

        const updatedValidatorEntry = updatedValidatorBalance.update(validatorEntryBuffer)
        if (updatedValidatorEntry === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to update validator entry.", node.from.key)
            return null;
        };

        const finalRequesterNodeEntry = this.#repo.stakeBalance(chargedFeeRequesterNodeEntry, node);
        if (finalRequesterNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to stake balance for writer.", node.from.key)
            return null;
        };
    ...
    async performOperation(op, view, base, node, batch) {
        ...
        if (!Object.hasOwn(op.rao, "vs") || !Object.hasOwn(op.rao, "va") || !Object.hasOwn(op.rao, "vn")) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Operation is not complete.", node.from.key)
            return Status.FAILURE;
        };

        // for additional security, nonces should be different.
        if (b4a.equals(op.rao.in, op.rao.vn)) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Nonces should not be the same.", node.from.key)
            return Status.FAILURE;
        };

        // addresses should be different.
        if (b4a.equals(op.address, op.rao.va)) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Addresses should be different.", node.from.key)
            return Status.FAILURE;
        };

        // signatures should be different.
        if (b4a.equals(op.rao.is, op.rao.vs)) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Signatures should be different.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (requesterAddressString === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Requester address is invalid.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(requesterPublicKey, NULL_BUFFER)) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Error while decoding requester public key.", node.from.key)
            return Status.FAILURE;
        };

        // if node want to register ZERO_WK, then this is NOT ALLOWED
        if (b4a.equals(op.rao.iw, ZERO_WK)) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Writer cannot initialize with zero-writer-key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (requesterMessage.length === 0) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Invalid requester message.", node.from.key)
            return Status.FAILURE;
        };

        const hash = await tracCryptoApi.hash.blake3Safe(requesterMessage);
        if (!b4a.equals(hash, op.rao.tx)) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Message hash does not match the tx_hash.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!isRequesterMessageVerifed) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to verify message signature.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (validatorAddressString === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to validate validator address.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(validatorPublicKey, NULL_BUFFER)) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to decode validator public key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (validatorMessage.length === 0) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Invalid validator message.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!isValidatorMessageVerifed) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to verify validator message signature.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (indexersSequenceState === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Indexer sequence state is invalid.", node.from.key)
            return Status.FAILURE;
        };

        if (!b4a.equals(op.rao.txv, indexersSequenceState)) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Transaction was not executed.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!isValidatorValid) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Validator consistency check failed.", node.from.key)
            return Status.FAILURE;
        }
        ...
        if (opEntry !== null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Operation has already been applied.", node.from.key)
            return Status.IGNORE;
        };
        ...
        if (addWriterResult === null) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Failed to add writer.", node.from.key)
            return Status.FAILURE;
        }

        if (addWriterResult === Status.IGNORE) {
            this.#repo.safeLog(OperationType.ADD_WRITER, "Add writer operation ignored.", node.from.key)
            return Status.IGNORE;
        }
