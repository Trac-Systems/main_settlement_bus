...
class AddIndexerHandler {
    ...
    async #addIndexer(op, node, batch, base, txHashHexString, pretendingAddressString, requesterAddressString) {
        ...
        if (pretenderNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to verify target indexer entry.", node.from.key)
            return null;
        };

        const decodedPretenderNodeEntry = nodeEntryUtils.decode(pretenderNodeEntry);
        if (decodedPretenderNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to decode pretender indexer node entry.", node.from.key)
            return null;
        };
        ...
        if (!isNodeWriter || isNodeIndexer) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Node must be a writer, and cannot already be an indexer.", node.from.key)
            return null;
        };
        ...
        if (updatedNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to update node role.", node.from.key)
            return null;
        };
        ...
        if (indexerListHasWk) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Writer key already exists in indexer list.", node.from.key)
            return null;
        ...
        if (feeAmount === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Invalid fee amount.", node.from.key)
            return null;
        };

        const adminNodeEntryBuffer = await this.#repo.getEntry(requesterAddressString, batch);
        if (adminNodeEntryBuffer === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Invalid requester node entry buffer.", node.from.key)
            return null;
        };

        const adminNodeEntry = nodeEntryUtils.decode(adminNodeEntryBuffer);
        if (adminNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to decode requester node entry.", node.from.key)
            return null;
        };

        const adminBalance = toBalance(adminNodeEntry.balance);
        if (adminBalance === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Invalid admin balance.", node.from.key)
            return null;
        };

        if (!adminBalance.greaterThanOrEquals(feeAmount)) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Insufficient requester balance.", node.from.key)
            return null;
        };
        ...
        if (newAdminBalance === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to apply fee to requester balance.", node.from.key)
            return null;
        };

        const updatedAdminNodeEntry = newAdminBalance.update(adminNodeEntryBuffer);
        if (updatedAdminNodeEntry === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to update requester node.", node.from.key)
            return null;
        };
    ...
    async performOperation(op, view, base, node, batch) {
        ...
        if (requesterAddressString === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Requester address is invalid.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(requesterPublicKey, NULL_BUFFER)) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Error while decoding requester public key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (pretendingAddressString === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Pretending indexer address is invalid.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(pretentingPublicKey, NULL_BUFFER)) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to decode pretending indexer public key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (adminEntry === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Invalid admin entry.", node.from.key)
            return Status.FAILURE;
        };

        const decodedAdminEntry = adminEntryUtils.decode(adminEntry, this.#config.addressPrefix);
        if (decodedAdminEntry === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to decode admin entry.", node.from.key)
            return Status.FAILURE;
        };

        if (!this.#repo.isAdmin(decodedAdminEntry, node)) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Node is not allowed to perform this operation. (ADMIN ONLY)", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(adminPublicKey, NULL_BUFFER)) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to decode admin public key.", node.from.key)
            return Status.FAILURE;
        };

        // Admin consistency check
        if (!b4a.equals(adminPublicKey, requesterPublicKey)) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "System admin and node public keys do not match.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (message.length === 0) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Invalid requester message.", node.from.key)
            return Status.FAILURE;
        };

        const hash = await tracCryptoApi.hash.blake3Safe(message);
        if (!b4a.equals(hash, op.aco.tx)) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Message hash does not match the tx_hash.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!isMessageVerified) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Failed to verify message signature.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (indexersSequenceState === null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Indexer sequence state is invalid.", node.from.key)
            return Status.FAILURE;
        };

        if (!b4a.equals(op.aco.txv, indexersSequenceState)) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Transaction was not executed.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (opEntry !== null) {
            this.#repo.safeLog(OperationType.ADD_INDEXER, "Operation has already been applied.", node.from.key)
            return Status.FAILURE;
        };
