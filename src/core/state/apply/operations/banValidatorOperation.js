...
class BanValidatorHandler extends BaseHandler {
    #repo;
    #config;
    ...
    async performOperation(op, view, base, node, batch) {
        if (!this.#stateValidationSchema.validateAdminControlOperation(op)) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Contract schema validation failed.", node.from.key)
        ...
        if (requesterAddressString === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Requester address is invalid.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (b4a.equals(requesterPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Error while decoding requester public key.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (adminEntry === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Invalid admin entry.", node.from.key)
            return Status.FAILURE;
        };

        const decodedAdminEntry = adminEntryUtils.decode(adminEntry, this.#config.addressPrefix);
        if (decodedAdminEntry === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to decode admin node entry.", node.from.key)
            return Status.FAILURE;
        };

        const adminPublicKey = tracCryptoApi.address.decodeSafe(decodedAdminEntry.address);
        if (b4a.equals(adminPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to decode admin public key.", node.from.key)
            return Status.FAILURE;
        };

        if (!this.#repo.isAdmin(decodedAdminEntry, node)) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Node is not allowed to perform this operation. (ADMIN ONLY)", node.from.key)
            return Status.FAILURE;
        };

        // Admin consistency check
        if (!b4a.equals(adminPublicKey, requesterPublicKey)) {
            this.logger.error(OperationType.BAN_VALIDATOR, "System admin and node public keys do not match.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (message.length === 0) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Invalid requester message.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!b4a.equals(regeneratedHash, op.aco.tx)) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Message hash does not match the tx_hash.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (!isMessageVerified) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to verify message signature.", node.from.key)
            return Status.FAILURE;
        }
        ...
        if (indexersSequenceState === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Indexer sequence state is invalid.", node.from.key)
            return Status.FAILURE;
        }

        if (!b4a.equals(op.aco.txv, indexersSequenceState)) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Transaction was not executed.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (opEntry !== null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Operation has already been applied.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (nodeToBeBannedAddressString === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to verify target node address.", node.from.key)
            return Status.FAILURE;
        };

        const toBanNodeEntry = await this.#repo.getEntry(nodeToBeBannedAddressString, batch);
        if (toBanNodeEntry === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to verify target node entry.", node.from.key)
            return Status.FAILURE;
        ...
        if ((!isWhitelisted && !isWriter) || isIndexer) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Only writer/whitelisted node can be banned.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (updatedToBanNodeEntry === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to update target node role.", node.from.key)
            return Status.FAILURE;
        };

        const decodedToBanNodeEntry = nodeEntryUtils.decode(updatedToBanNodeEntry);
        if (decodedToBanNodeEntry === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to decode target node entry.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (feeAmount === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Invalid fee amount.", node.from.key)
            return Status.FAILURE;
        };

        const adminNodeEntryBuffer = await this.#repo.getEntry(requesterAddressString, batch);
        if (adminNodeEntryBuffer === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Invalid admin node entry buffer.", node.from.key)
            return Status.FAILURE;
        };

        const adminNodeEntry = nodeEntryUtils.decode(adminNodeEntryBuffer);
        if (adminNodeEntry === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to verify admin node entry.", node.from.key)
            return Status.FAILURE;
        };

        const adminBalance = toBalance(adminNodeEntry.balance);
        if (adminBalance === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Invalid admin balance", node.from.key)
            return Status.FAILURE;
        };

        if (!adminBalance.greaterThanOrEquals(feeAmount)) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Insufficient admin balance.", node.from.key)
            return Status.FAILURE;
        };
        ...
        if (newAdminBalance === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to apply fee to admin balance.", node.from.key)
            return Status.FAILURE;
        };

        const updatedAdminNodeEntry = newAdminBalance.update(adminNodeEntryBuffer);
        if (updatedAdminNodeEntry === null) {
            this.logger.error(OperationType.BAN_VALIDATOR, "Failed to update admin node balance.", node.from.key)
            return Status.FAILURE;
        }

        // Remove the writer role and update the state
        if (isWriter) {
            const finalNodeEntry = this.withdrawStakedBalance(updatedToBanNodeEntry, node);
            if (finalNodeEntry === null) {
                this.logger.error(OperationType.BAN_VALIDATOR, "Failed to withdraw staked balance.", node.from.key)
                return Status.FAILURE;
            }
