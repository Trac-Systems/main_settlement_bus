...
class SetConsensusConfigHandler extends BaseHandler {
    #repo;
    #config;
    ...
    async performOperation(op, view, base, node, batch) {
        if (!this.#stateValidationSchema.validateConsensusControlOperation(op)) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Contract schema validation failed.", node.from.key)
        ...
        if (requesterAddressString === null) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Requester address is invalid.", node.from.key)
            return Status.FAILURE;
        }
        ...
        if (b4a.equals(requesterPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Failed to decode requester public key.", node.from.key)
            return Status.FAILURE;
        }

        const adminEntry = await this.#repo.getEntry(EntryType.ADMIN, batch);
        if (adminEntry === null) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Invalid admin entry.", node.from.key)
            return Status.FAILURE;
        }

        const decodedAdminEntry = adminEntryUtils.decode(adminEntry, this.#config.addressPrefix);
        if (decodedAdminEntry === null) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Failed to decode admin entry.", node.from.key)
            return Status.FAILURE;
        }

        if (!this.#repo.isAdmin(decodedAdminEntry, node)) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Node is not allowed to perform this operation. (ADMIN ONLY)", node.from.key)
            return Status.FAILURE;
        }

        const adminPublicKey = tracCryptoApi.address.decodeSafe(decodedAdminEntry.address);
        if (b4a.equals(adminPublicKey, NULL_BUFFER)) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Failed to decode admin public key.", node.from.key)
            return Status.FAILURE;
        }

        if (!b4a.equals(adminPublicKey, requesterPublicKey)) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "System admin and node public keys do not match.", node.from.key)
            return Status.FAILURE;
        }
        ...
        if (encodedConsensusConfig.length === 0) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Failed to encode consensus config.", node.from.key);
            return Status.FAILURE;
        }

        if (!this.#repo.validateConsensusConfig(op.cco.cc)) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Consensus config validation failed.", node.from.key);
            return Status.FAILURE;
        }
        ...
        if (message.length === 0) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Invalid requester message.", node.from.key)
            return Status.FAILURE;
        }

        const hash = await tracCryptoApi.hash.blake3Safe(message);
        if (!b4a.equals(hash, op.cco.tx)) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Message hash does not match the tx_hash.", node.from.key)
            return Status.FAILURE;
        }

        const isMessageVerified = tracCryptoApi.signature.verify(op.cco.is, op.cco.tx, adminPublicKey);
        if (!isMessageVerified) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Failed to verify message signature.", node.from.key)
            return Status.FAILURE;
        }

        const indexersSequenceState = await this.#repo.getIndexerSequenceState(base);
        if (indexersSequenceState === null) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Indexer sequence state is invalid.", node.from.key)
            return Status.FAILURE;
        }

        if (!b4a.equals(op.cco.txv, indexersSequenceState)) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Transaction was not executed.", node.from.key)
            return Status.FAILURE;
        }
        ...
        if (opEntry !== null) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Operation has already been applied.", node.from.key)
        ...
        if (currentConsensusConfigBuffer === null) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Initial consensus config has not been initialized yet", node.from.key)
            return Status.IGNORE;
        }

        const currentConsensusConfigIndex = safeReadUint32BE(currentConsensusConfigBuffer);
        if (currentConsensusConfigIndex === null) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG,"Failed to read current consensus config index from buffer", node.from.key)
            return Status.FAILURE;
        }
        if (currentConsensusConfigIndex === UINT32_MAX) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Consensus config index overflow.", node.from.key)
            return Status.FAILURE;
        }
        ...
        if (nextConsensusConfigIndexBuffer.length === 0) {
            this.logger.error(OperationType.SET_CONSENSUS_CONFIG, "Failed to encode next consensus config index.", node.from.key);
            return Status.FAILURE;
        }
