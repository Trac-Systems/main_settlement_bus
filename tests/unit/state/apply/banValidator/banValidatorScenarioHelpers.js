...
export async function buildBanValidatorPayload(
	...
	return await new CompleteStateMessageOperations(adminPeer.wallet, config).assembleBanWriterMessage(
...
export async function promoteValidatorToIndexer(
	...
	const txValidity = await deriveIndexerSequenceState(adminPeer.base);
	const payload = await new CompleteStateMessageOperations(adminPeer.wallet, config).assembleAddIndexerMessage(
		validatorPeer.wallet.address,
		txValidity
	);
