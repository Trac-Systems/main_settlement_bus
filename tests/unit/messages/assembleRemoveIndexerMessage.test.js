...
test(testName, async (t) => {
    ...
    const assembler = async (wallet, address) => {
        const payload = await createApplyStateMessageFactory(wallet, config)
            .buildCompleteRemoveIndexerMessage(wallet.address, address, txValidity);
        return safeDecodeApplyOperation(safeEncodeApplyOperation(payload));
    }

    await messageOperationsBkoTest(t, testName, assembler, walletAdmin, writingKeyNonAdmin, OperationType.REMOVE_INDEXER, 5, walletNonAdmin.address);
});
